return {
  ...
  name = "ts-ref-codelens",
  lazy = false,
  config = function()
    local ns = vim.api.nvim_create_namespace "ts_ref_codelens"
    local filetypes = { typescript = true, javascript = true, typescriptreact = true, javascriptreact = true }
    ...
    -- BufEnter fires before LSP attaches on first open, so also trigger on attach
    ...
    vim.api.nvim_create_user_command("TsRefresh", function() refresh() end, { desc = "Refresh TS reference counts" })
    ...
    vim.api.nvim_create_user_command("TsRefDebug", function()
    ...
    end, { desc = "Debug TS reference codelens" })
  end,
}
